...
function getGitAPI(): VSCodeGit.API {
  ...
  if (!vscodeGit?.exports.getAPI(1)) {
    output.error("getGitAPI", "vscode.git is not enabled.", true);
  }
...
async function getRepository({
  ...
  if (_arg) {
    ...
    if (repo) {
      ...
      output.error(
        "getRepository",
        "Repository not found in path: " + _arg,
        true
      );
    }
  }
  ...
  if (git.repositories.length === 0) {
    output.error("getRepository", "Please open a repository.", true);
  }
